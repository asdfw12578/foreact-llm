...
class ActionDataset(Data.Dataset):
    def __init__(self, args, mode,vid_list,actions_dict,features_path,text_feature_path,gt_path,dataset,n_class, pad_idx):
        ...
        if self.mode=='train':
            if self.dataset == 'breakfast':
                ...
                # Generic fallback for additional datasets. This mirrors the
                # Breakfast protocol and keeps the first run simple/stable.
                # If the new dataset has very long videos, we can later switch
                # to the 50Salads-style start-frame augmentation.
                start_frame = 0
                for vid in vid_list:
                    self.vid_list.append([vid, .2, start_frame])
                    self.vid_list.append([vid, .3, start_frame])
    ...
    def _make_input(self, vid_file, obs_perc, start_frame):
        ...
        trans_future = np.append(trans_future, self.NONE)  # 灏?self.NONE 娣诲姞鍒?trans_future 鏁扮粍鐨勬湯灏?        trans_future_target = trans_future  # target
