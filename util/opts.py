...
def get_args_parser():
    ...
    parser.add_argument('--gtda_enable', action='store_true',
                        help='Enable GTDA-style temporal gated stack in visual branch before CMIB')
    parser.add_argument('--gtda_layers', type=int, default=4,
                        help='Number of temporal gated residual layers')
    parser.add_argument('--gtda_kernel_size', type=int, default=3,
                        help='Kernel size for temporal gated convolutions')
    parser.add_argument('--gtda_dropout', type=float, default=0.1,
                        help='Dropout in temporal gated stack')
    parser.add_argument('--gtda_causal', action='store_true',
                        help='Use strict causal temporal convolutions in GTDA gated stack')
    parser.add_argument('--gtda_res_scale_init', type=float, default=1e-3,
                        help='Initial residual scale for temporal gated output')
    parser.add_argument('--local_aux_enable', action='store_true',
                        help='Enable local auxiliary supervision on observed history')
    parser.add_argument('--local_aux_action_weight', type=float, default=0.1,
                        help='Weight for observed local action auxiliary CE loss')
    parser.add_argument('--local_aux_boundary_weight', type=float, default=0.0,
                        help='Weight for observed boundary auxiliary BCE loss')
    parser.add_argument('--local_aux_boundary_pos_weight', type=float, default=5.0,
                        help='Positive weight for boundary BCE to mitigate class imbalance')
    parser.add_argument('--local_aux_warmup_epochs', type=float, default=0.0,
                        help='Warm up local auxiliary weights to target values in first N epochs')
    parser.add_argument('--local_aux_start_epoch', type=float, default=0.0,
                        help='Epoch index to start increasing local auxiliary weights')
    parser.add_argument('--local_aux_tail_ratio', type=float, default=1.0,
                        help='Use only the last ratio of valid observed tokens for local auxiliary losses (0,1]')
    parser.add_argument('--sdr_resampler_enable', action='store_true',
                        help='Enable vision token resampler before CMIB attention')
    parser.add_argument('--sdr_resampler_num_tokens', type=int, default=8,
                        help='Number of latent tokens kept by the vision resampler')
    parser.add_argument('--sdr_resampler_layers', type=int, default=1,
                        help='Number of cross-attention layers in the vision resampler')
    parser.add_argument('--sdr_res_scale_init', type=float, default=1e-3,
                        help='Initial residual scale for SDR resampler feedback')
    parser.add_argument('--sdr_semantic_weight_enable', action='store_true',
                        help='Enable semantic-density-aware text attention bias')
    parser.add_argument('--sdr_semantic_alpha', type=float, default=0.1,
                        help='Scale for semantic-density text attention bias')
    parser.add_argument('--disable_text_features', action='store_true',
                        help='Disable text feature inputs and text auxiliary loss, useful for datasets without text features')
