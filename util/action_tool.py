...
def read_mapping_dict(file_path):
    ...
    for a in actions:
        ...
        if len(parts) < 2:
            continue
        # Action names in Assembly101 coarse annotations can contain spaces
        # (e.g. "detach track"), so only the first token is the class id.
        actions_dict[" ".join(parts[1:])] = int(parts[0])
...
def backup_code(code_version):
    ...
    fol_list = ['data','foreactllm','util','scripts']
