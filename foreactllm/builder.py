...
import json
from foreactllm.model import ModelArgs, Transformer
from foreactllm.tokenizer import Tokenizer
from foreactllm.mm_adapter import set_MMAdapter
...
def ActionLLM(args):
    ...
    model_args: ModelArgs = ModelArgs(
        ...
        multi_hidden_proj=args.multi_hidden_proj,
        gtda_enable=args.gtda_enable,
        gtda_layers=args.gtda_layers,
        gtda_kernel_size=args.gtda_kernel_size,
        gtda_dropout=args.gtda_dropout,
        gtda_causal=args.gtda_causal,
        gtda_res_scale_init=args.gtda_res_scale_init,
        local_aux_enable=args.local_aux_enable,
        local_aux_action_weight=args.local_aux_action_weight,
        local_aux_boundary_weight=args.local_aux_boundary_weight,
        local_aux_boundary_pos_weight=args.local_aux_boundary_pos_weight,
        local_aux_tail_ratio=args.local_aux_tail_ratio,
        sdr_resampler_enable=args.sdr_resampler_enable,
        sdr_resampler_num_tokens=args.sdr_resampler_num_tokens,
        sdr_resampler_layers=args.sdr_resampler_layers,
        sdr_res_scale_init=args.sdr_res_scale_init,
        sdr_semantic_weight_enable=args.sdr_semantic_weight_enable,
        sdr_semantic_alpha=args.sdr_semantic_alpha,
        disable_text_features=getattr(args, "disable_text_features", False),
        **params
    )
    ...
    learnable_keys=['adapter', 'gtda', 'local', 'sdr']
    total=0.
    trainable_names=[]
    for name, param in llama.named_parameters():
        is_trainable = any(key in name for key in learnable_keys)
        if is_trainable:
    ...
    print('  + Trainable tensors: %d' % len(trainable_names))
    for name in trainable_names:
        if ('local' in name) or ('gtda' in name) or ('sdr' in name):
            print('    -', name)
    return llama
