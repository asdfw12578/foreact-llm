...
from util.action_dataset_assembly101 import Assembly101ActionDataset
...
def set_local_aux_weights(model, args, epoch):
    core = model.module if hasattr(model, "module") else model
    if not hasattr(core, "local_aux_enable") or not core.local_aux_enable:
        return
    target_action = float(args.local_aux_action_weight)
    target_boundary = float(args.local_aux_boundary_weight)
    warmup_epochs = float(args.local_aux_warmup_epochs)
    start_epoch = float(args.local_aux_start_epoch)
    if warmup_epochs <= 0:
        ratio = 1.0
    else:
        ratio = (float(epoch) - start_epoch + 1.0) / warmup_epochs
        ratio = max(0.0, min(1.0, ratio))
    core.local_aux_action_weight = target_action * ratio
    core.local_aux_boundary_weight = target_boundary * ratio
    print(
        f"Local aux weights @epoch{epoch}: "
        f"ratio={ratio:.4f}, "
        f"action_w={core.local_aux_action_weight:.6f}, "
        f"boundary_w={core.local_aux_boundary_weight:.6f}"
    )
...
def main(args):
    ...
    if dataset == 'breakfast':
        ...
        # Allow new datasets to reuse the SDR pipeline as long as they follow
        # the standard ActionLLM directory layout under args.data_root.
    ...
    if dataset == 'assembly101':
        dataset_train = Assembly101ActionDataset(args, 'train', actions_dict, data_path, n_class, pad_idx)
        dataset_val = Assembly101ActionDataset(args, 'val', actions_dict, data_path, n_class, pad_idx)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        set_local_aux_weights(model, args, epoch)
