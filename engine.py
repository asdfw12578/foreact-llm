...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (item) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if not torch.isfinite(loss):
            print("Non-finite loss encountered. Skipping this batch and clearing grads.")
...
def predict(model, vid_list, args, obs_p, n_class, actions_dict, device, data_path,res_des,text_feature_path, feature_cache=None):
    model.eval()
    with torch.no_grad():
        ...
        for vid in vid_list:
            ...
            cached = feature_cache.get(file_name) if feature_cache is not None else None
            if cached is None:
                # Load once when a shared cache is provided by multi-checkpoint evaluation.
                gt_file = os.path.join(gt_path, file_name+'.txt')
                with open(gt_file, 'r') as gt_read:
                ...
                if feature_cache is not None:
                    feature_cache[file_name] = (gt_seq, features, text_features)
            else:
                gt_seq, features, text_features = cached
